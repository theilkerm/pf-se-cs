...
export default function CartPage() {
    ...
    const getCart = useCallback(async () => {
        ...
        try {
            ...
            // If we get rate limited, the cart will appear empty.
            setCartItems([]);
        } finally {
            setLoading(false);
        }
    ...
    const handleClearCart = async () => {
        if (!token || !confirm("Are you sure you want to clear your entire cart?")) return;
        try {
            await fetcher('/cart', {
                method: 'DELETE',
                headers: { Authorization: `Bearer ${token}` }
    ...
    const handleUpdateQuantity = (productId: string, newQuantityStr: string) => {
        ...
        // Prevent updating UI to an invalid state (e.g., more than stock)
        // We allow it to be temporarily empty/NaN for better UX while typing
        if (newQuantity > item.product.stock) {
            alert(`Cannot add more than ${item.product.stock} items.`);
            return;
        }

        setCartItems(currentItems => currentItems.map(cartItem =>
            cartItem.product._id === productId ? { ...cartItem, quantity: isNaN(newQuantity) ? 1 : newQuantity } : cartItem
        ));

        debounceTimeout.current = setTimeout(async () => {
            // Final validation before sending to API
            if (!token || isNaN(newQuantity) || newQuantity < 1) {
                getCart(); // Revert to server state if input is invalid
                return;
            }
            try {
                await fetcher('/cart', {
    ...
    return (
        <div className="container mx-auto px-4 py-8">
            <div className="flex justify-between items-center mb-6">
                ...
                {cartItems.length > 0 && (
                    <button onClick={handleClearCart} className="bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600 font-semibold">
                        Clear Entire Cart
                    </button>
                )}
            </div>
                            ...
                            {cartItems.map(item => (
                                <div key={item.product._id} className="flex items-center p-4 border-b last:border-b-0">
                                    ...
                                    <div className="text-right">
                                        ...
                                        <button onClick={() => handleRemoveItem(item.product._id)} className="text-red-500 hover:text-red-700 text-sm mt-1">
                                            Remove
                                        </button>
                                    </div>
                                </div>
