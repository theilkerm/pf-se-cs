            ...
            <div className="bg-white p-6 shadow-md rounded-lg">
                ...
                <div className="space-y-4">
                    {order.orderItems.map((item, index) => {
                        const imageUrl = item.image && !item.image.includes('placeholder')
                            ? `${process.env.NEXT_PUBLIC_API_URL}${item.image}`.replace('/api/v1', '')
                            : `https://placehold.jp/150x150.png?text=${encodeURIComponent(item.name)}`;

                        return (
                            <div key={index} className="flex items-center py-2 border-b last:border-b-0">
                               <div className="relative w-[60px] h-[60px]">
                                    <Image src={imageUrl} alt={item.name} fill className="rounded object-cover" />
                               </div>
